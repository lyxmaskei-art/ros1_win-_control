    ...
    try:
        ...
        for step in range(total_steps):
            ...
            record_history(
                ...
                joint_velocity=result.get('theta_dot_next'),
                joint_velocity_raw=result.get('theta_dot_raw'),
            )

            send_joint_velocity_commands(client_id, joint_handles, result['theta_dot_next'])
            sim.simxSynchronousTrigger(client_id)
            sim.simxGetPingTime(client_id)
...
def parse_args():
    ...
    parser.add_argument('--tau', type=float, default=0.005, help='Control time step (s). Keep the original 200 Hz experiment step.')
    ...
    parser.add_argument('--real-command-backend', choices=['ros_position_trajectory'], default='ros_position_trajectory', help='Real UR3e transport backend.')
    ...
    parser.add_argument('--ur3e-servoj-t', type=float, default=0.005, help='ROS position trajectory control-period parameter.')
    ...
    parser.add_argument('--ur3e-speedj-t', type=float, default=0.005, help='ROS velocity control-period parameter.')
