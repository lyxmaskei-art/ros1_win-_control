...
class ContinuousTVQPELNCPController:
    ...
    def _internal_disturbance_vector(self, t_current, size):
        """Additive neural-dynamics disturbance n(t) following the strict paper convention."""
        ...
        return float(getattr(self.cfg, 'disturbance_scale', 1.0)) * profile * np.ones(size, dtype=float)
...
def run_live(method_name, controller_builder, robot, settings, output_dir, use_feedback=True, args=None):
    return run_ros_position_live(
        node_name='run_mild_disturbance_ros_real',
        ...
        step_accepts_time=True,
        experiment_label='robust',
    )
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default='none',
        help='Additive internal neural-dynamics disturbance: constant n1=5, linear n2=0.5t, quadratic n3=0.5(t-5)^2.',
    )
...
def main():
    ...
    output_root = Path(args.output_root) if args.output_root else PROJECT_ROOT / 'results' / 'robust' / datetime.now().strftime("%Y%m%d_%H%M%S")
