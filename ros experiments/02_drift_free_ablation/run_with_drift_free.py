...
def run_live(method_name, controller_builder, robot, settings, output_dir, use_feedback=True, args=None):
    return run_ros_position_live(
        node_name='run_with_drift_free_ros_real',
...
def main():
    ...
    output_root = Path(args.output_root) if args.output_root else PROJECT_ROOT / 'results' / 'with_drift' / datetime.now().strftime("%Y%m%d_%H%M%S")
