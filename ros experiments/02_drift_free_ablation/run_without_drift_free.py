...
class UR3eKinematics:
    def __init__(self):
        ...
        self.num_joints = 6
        self.tool_offset = np.zeros(3, dtype=float)
    ...
    def forward_kinematics(self, theta, return_intermediate=False):
        ...
        if return_intermediate:
            return transforms
        tcp_transform = transforms[-1].copy()
        tcp_transform[:3, 3] = tcp_transform[:3, 3] + tcp_transform[:3, :3] @ np.asarray(self.tool_offset, dtype=float)
        return tcp_transform

    def jacobian(self, theta):
        transforms = self.forward_kinematics(theta, return_intermediate=True)
        flange_transform = transforms[-1]
        o_n = flange_transform[:3, 3] + flange_transform[:3, :3] @ np.asarray(self.tool_offset, dtype=float)
...
class CircleTrajectory:
    def __init__(self, duration, radius=0.07, offset=np.array([0.3, 0.0, 0.4], dtype=float)):
        self.duration = float(duration)
...
def build_trajectory(name, duration, scale, initial_position):
    ...
    if name == 'circle':
        return CircleTrajectory(duration=duration, radius=0.07, offset=initial_position), 'circle'
...
def startup_handshake_and_settle(
    ...
    theta_measured = theta_goal.copy()
...
def record_history(history, tk, theta_current, sim_control_pos, fk_pos, sim_tip_pos, desired_pos, theta_reference, task_residual=None,
                   ...
                   joint_velocity_raw=None, joint_position_target=None, joint_velocity_feedback=None):
    ...
    if joint_position_target is None:
        joint_position_target = np.asarray(theta_current, dtype=float)
    if joint_velocity_feedback is None:
        joint_velocity_feedback = np.zeros_like(theta_current, dtype=float)
    ...
    history['joint_position_targets'].append(np.asarray(joint_position_target, dtype=float).tolist())
    ...
    history['joint_velocity_feedback'].append(np.asarray(joint_velocity_feedback, dtype=float).tolist())
...
class ContinuousTVQPELNCPController:
    ...
    def _build_residual(self, y_state, jacobian_task, task_command, lower, upper):
        ...
        stationarity = self.weight_matrix @ u - jacobian_task.T @ lam + sigma * omega
    ...
    def _estimate_time_derivatives(self, jacobian_task, task_command, lower, upper):
    ...
    def step(self, theta_current, desired_position, desired_velocity, use_feedback=True):
        ...
        residual_pack = self._build_residual(y_state=y_state, jacobian_task=jacobian_task, task_command=task_command, lower=lower, upper=upper)
        ...
        for _ in range(inner_steps):
            residual_pack = self._build_residual(y_state=y_next, jacobian_task=jacobian_task, task_command=task_command, lower=lower, upper=upper)
            u = residual_pack['u']
            lam = residual_pack['lambda']
            alpha = residual_pack['alpha']
            use_lower_branch = residual_pack['use_lower_branch']

            r1_t = -derivatives['jacobian_dot'].T @ lam
...
def build_method2_dlccznn(robot, settings):
    ...
    controller.cfg.drift_gain = 0.0
    controller.cfg.drift_free_term = 'removed_from_residual'
    return controller
...
def apply_config_overrides(controller, args):
    ...
    controller.cfg.drift_gain = 0.0
    if args.drift_gain is not None and not getattr(args, 'no_drift_criterion', False):
...
def run_live(method_name, controller_builder, robot, settings, output_dir, use_feedback=True, args=None):
    return run_ros_position_live(
        node_name='run_without_drift_free_ros_real',
...
def make_settings(args, trajectory_name=None, tau=None, pdnn_inner_steps=None, dlccznn_inner_steps=None):
    ...
    if hasattr(args, 'effective_theta_initial'):
        settings.theta_initial_command = np.asarray(args.effective_theta_initial, dtype=float).copy()
    return settings
...
def parse_args():
    ...
    parser.add_argument('--drift-gain', type=float, default=None, help='Ignored when no drift criterion is requested.')
    parser.add_argument('--no-drift-criterion', action='store_true', help='Disable joint drift feedback term')
...
def main():
    ...
    output_root = Path(args.output_root) if args.output_root else PROJECT_ROOT / 'results' / 'no_drift' / datetime.now().strftime("%Y%m%d_%H%M%S")
    ...
    if args.experiment == 'single':
        ...
        if args.method == 'method2_dlccznn' and args.no_drift_criterion:
            settings.drift_gain = 0.0
