...
def plot_joint_velocities(history, output_path, title_prefix):
    time_vec = np.asarray(history['time_s'], dtype=float)
    joints = np.asarray(history['joint_positions'], dtype=float)
    if joints.shape[0] >= 2 and time_vec.shape[0] == joints.shape[0]:
        dt = np.diff(time_vec)
        valid_dt = np.where(np.abs(dt) > 1e-12, dt, np.nan)
        velocity_diff = np.diff(joints, axis=0) / valid_dt[:, None]
        velocities = np.vstack((velocity_diff, velocity_diff[-1]))
    else:
...
def run_live(method_name, controller_builder, robot, settings, output_dir, use_feedback=True, args=None):
    return run_ros_position_live(
        node_name='run_clean_repetitive_tracking_ros_real',
...
def main():
    ...
    output_root = Path(args.output_root) if args.output_root else PROJECT_ROOT / 'results' / 'clean' / datetime.now().strftime("%Y%m%d_%H%M%S")
