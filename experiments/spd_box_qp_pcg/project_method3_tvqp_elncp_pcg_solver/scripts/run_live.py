...
def run_live_experiment(cfg, robot, output_dir, save_artifacts=True):
    ...
    try:
        ...
        for step in range(cfg.steps):
            ...
            record_history(history, tk, theta_current, current_pos_after, desired_pos, theta_reference, step_result)
    ...
    summary = summarize_run("Method 3 TVQP ELNCP PCG live", history, theta_reference, theta_current, output_dir, cfg, port=port)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Integrated Method 3 continuous TVQP + ELNCP + PCG runner.")
