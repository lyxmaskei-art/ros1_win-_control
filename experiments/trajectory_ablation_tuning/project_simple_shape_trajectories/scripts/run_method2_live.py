...
METHOD_NAME = "Method 2: Scheme-B outer setting with DLCCZNN solver"
OUTPUT_STEM = "method2_scheme_b_positive_dlccznn_shapes"
...
class CircleTrajectory:
    ...
    def get_pose(self, t):
        ...
        # Quintic rest-to-rest timing so the full circle starts and ends with zero velocity.
...
class LineTrajectory:
    ...
    def get_pose(self, t):
        u = np.clip(float(t) / self.duration, 0.0, 1.0)
        # Cosine out-and-back profile on a straight line with zero boundary velocity.
...
def run_live_experiment(cfg, robot, output_dir):
    ...
    try:
        ...
        trajectory, trajectory_tag = build_trajectory(cfg.trajectory_name, cfg.duration, cfg.heart_scale, initial_pos)
    ...
    save_figures(history, output_dir, f"{METHOD_NAME} Live ({trajectory_tag})", theta_reference, theta_current, "live")
    summary = summarize_run(f"Method 2 DLCCZNN live ({trajectory_tag})", history, theta_reference, theta_current, output_dir, port=port)
...
def main():
    ...
    output_root = base_root if args.output_root else build_run_output_root(base_root, args, "live")
    output_root.mkdir(parents=True, exist_ok=True)
    save_run_config(output_root, args, "live")

    print("Running live experiment ...")
    live_summary = run_live_experiment(cfg, robot, output_root / "live")
