...
def run_offline(method_name, controller_builder, robot, settings, output_dir, use_feedback=True, args=None):
    display_name = METHOD_DISPLAY.get(method_name, method_name)
    mode_label = 'with_fb' if use_feedback else 'nofb'
    stem = f'{method_name}_offline_{mode_label}'
    title_prefix = f'{display_name} Offline ({mode_label})'

    print(f'  [{display_name}] Running offline ({mode_label})...')
...
def main():
    ...
    output_root = Path(args.output_root) if args.output_root else build_run_output_root(PROJECT_ROOT / 'results' / fb_label, args, "offline")
    ...
    if args.experiment == 'single':
        ...
        summary = run_one_case(args.method, 'offline', robot, settings, case_root / 'offline', use_feedback, args)
        append_summary_row(all_rows, f'single_{args.trajectory_name}_{args.method}_offline', summary)
