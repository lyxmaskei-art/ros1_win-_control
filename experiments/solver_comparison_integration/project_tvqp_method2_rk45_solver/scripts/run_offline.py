...
def run_offline(method_name, controller_builder, robot, settings, output_dir, use_feedback=True):
    ...
    trajectory = HeartTrajectory(
        duration=settings.offline_duration,
...
def run_experiment(args):
    output_root = Path(args.output_root) if args.output_root else build_run_output_root(PROJECT_DIR / "results", args, "offline")
    ...
    _, summary = run_offline(
        ...
        robot,
        settings,
        output_root / "method2" / "offline",
        use_feedback=True,
    )
    if summary:
        collect_solver_diagnostics(
            output_root / "method2" / "offline",
            "method2_offline_with_fb",
            None,
            summary,
            settings,
        )
        all_summaries["method2_offline"] = summary
