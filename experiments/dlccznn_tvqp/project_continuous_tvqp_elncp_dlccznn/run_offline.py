...
def main():
    ...
    if args.output_root:
        ...
        output_root = build_run_output_root(CURRENT_DIR / f'results_{fb_label}', args, "offline")
    ...
    for method_name in methods_to_run:
        ...
        print(f'--- {METHOD_DISPLAY[method_name]}: Offline ---')
        _, summary = run_offline(
            method_name,
            builder,
            robot,
            settings,
            method_output_dir / 'offline',
            use_feedback=use_feedback,
            args=args,
        )
        if summary:
            all_summaries[f'{method_name}_offline'] = summary
