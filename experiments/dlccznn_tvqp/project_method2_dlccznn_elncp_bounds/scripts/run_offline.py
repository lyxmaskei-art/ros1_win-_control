...
METHOD_NAME = "Method 2: Scheme-B outer setting with DLCCZNN-ELNCP bound solver"
OUTPUT_STEM = "method2_scheme_b_positive_dlccznn_elncp"
...
class Method2DLCCZNNConfig:
    ...
    solver_substeps: int = 60
    solver_gamma_gain: float = 2560.0
...
class Method2DLCCZNNSolver:
    def __init__(self, cfg, num_primal=6, num_dual=3):
        self.cfg = cfg
        self.num_primal = int(num_primal)
        self.num_dual = int(num_dual)
        self.state = np.zeros(2 * self.num_primal + self.num_dual, dtype=float)

    def reset(self):
        self.state = np.zeros(2 * self.num_primal + self.num_dual, dtype=float)

    def build_elncp_residual_and_direction(self, y, h_matrix, p_vector, lower, upper):
        n = self.num_primal
        m = self.num_dual
        u = y[:n]
        lam = y[n:n + m]
        omega = np.maximum(y[n + m:n + m + n], 0.0)

        h_core = h_matrix @ np.concatenate((u, lam)) + p_vector
        lower_slack = u - lower
        upper_slack = upper - u
        use_lower = lower_slack <= upper_slack
        s_value = np.where(use_lower, lower_slack, upper_slack)
        ds_du = np.where(use_lower, 1.0, -1.0)
        sigma = -ds_du

        sqrt_term = np.sqrt(s_value * s_value + omega * omega + self.cfg.ncp_epsilon)
        pfb = s_value + omega - sqrt_term
        alpha = 1.0 - s_value / sqrt_term
        beta = 1.0 - omega / sqrt_term

        stationarity = h_core[:n] + sigma * omega
        equality = h_core[n:n + m]
        residual = np.concatenate((stationarity, equality, pfb))

        direction_u = h_matrix[:n, :n].T @ stationarity
        direction_u += h_matrix[n:n + m, :n].T @ equality
        direction_u += alpha * ds_du * pfb
        direction_lambda = h_matrix[:n, n:n + m].T @ stationarity
        direction_lambda += h_matrix[n:n + m, n:n + m].T @ equality
        direction_omega = sigma * stationarity + beta * pfb
        direction = np.concatenate((direction_u, direction_lambda, direction_omega))
        return residual, direction, u, omega, s_value
    ...
    def step(self, h_matrix, p_vector, lower, upper):
        ...
        y = self.state.copy()
        y_raw = y.copy()
        residual, _, _, _, _ = self.build_elncp_residual_and_direction(y, h_matrix, p_vector, lower, upper)

        for _ in range(max(int(self.cfg.solver_substeps), 1)):
            residual, direction, _, _, _ = self.build_elncp_residual_and_direction(y, h_matrix, p_vector, lower, upper)
            ...
            y = y_raw.copy()
            y[self.num_primal + self.num_dual :] = np.maximum(y[self.num_primal + self.num_dual :], 0.0)
            if not np.all(np.isfinite(y)):
                raise FloatingPointError("Method 2 DLCCZNN solver diverged; reduce solver gain.")

        self.state = y.copy()
        final_residual, _, u_final, omega_final, s_value = self.build_elncp_residual_and_direction(y, h_matrix, p_vector, lower, upper)
        return {
            "theta_dot": u_final.copy(),
            "theta_dot_raw": y_raw[: self.num_primal].copy(),
            "dual_next": y[self.num_primal : self.num_primal + self.num_dual].copy(),
            "omega_next": omega_final.copy(),
...
class Method2Controller:
    def __init__(self, cfg):
        ...
        self.solver = Method2DLCCZNNSolver(cfg)
    ...
    def step(self, theta_current, current_pos, desired_pos, desired_vel, jacobian_task, lower, upper):
        ...
        return {
            ...
            "omega_next": solver_output["omega_next"],
            "min_boundary_slack": solver_output["min_boundary_slack"],
...
def record_history(history, tk, theta_current, current_pos, desired_pos, theta_reference, inner_residual_norm, boundary_slack):
    theta_current = np.asarray(theta_current, dtype=float)
    ...
    history["boundary_slack"].append(float(boundary_slack))
...
def summarize_run(label, history, theta_initial, theta_final, result_dir, port=None):
    ...
    summary = {
        ...
        "min_boundary_slack": float(np.min(np.asarray(history["boundary_slack"], dtype=float))),
        "final_boundary_slack": float(np.asarray(history["boundary_slack"], dtype=float)[-1]),
...
def run_offline_experiment(cfg, robot, output_dir, save_artifacts=True):
    output_dir.mkdir(parents=True, exist_ok=True)
    controller = Method2Controller(cfg)
    ...
    for step in range(steps):
        ...
        record_history(
            ...
            step_result["min_boundary_slack"],
        )
    wall_clock = time.perf_counter() - t_start

    if save_artifacts:
        save_figures(history, output_dir, METHOD_NAME + " Offline", theta_reference, theta_current, "offline")
    summary = summarize_run("Method 2 DLCCZNN offline", history, theta_reference, theta_current, output_dir)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Integrated Method 2 DLCCZNN project runner.")
    ...
    parser.add_argument("--mu-gain", type=float, default=10.0)
    ...
    parser.add_argument("--solver-substeps", type=int, default=60)
    parser.add_argument("--solver-gamma-gain", type=float, default=2560.0)
...
def main():
    ...
    offline_summary = run_offline_experiment(
        cfg,
        robot,
        output_root / "offline",
