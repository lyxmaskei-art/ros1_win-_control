...
def run_live_noise_trial(cfg, robot, noise_channel, noise_profile, noise_level, trial, seed, output_dir, save_artifacts):
    ...
    try:
        ...
        _, theta_reference = startup_handshake_and_settle(
            client_id, joint_handles, cfg.theta_initial_command, cfg.tau
        )
        ...
        for step in range(cfg.steps):
            tk = step * cfg.tau
            envelope = noise_envelope(noise_profile, tk, cfg.duration)
            theta_feedback = read_joint_positions_fast(client_id, joint_handles)
            ...
            theta_sensed = theta_feedback + sensor_noise
            ...
            step_result = controller.step(
                theta_sensed, current_pos_sensed, desired_pos, desired_vel, jacobian_sensed, lower, upper
            )

            theta_dot_actual = step_result["theta_dot"] + actuator_noise
            theta_target = np.clip(theta_feedback + cfg.tau * theta_dot_actual, cfg.theta_lower, cfg.theta_upper)
            send_joint_targets(client_id, joint_handles, theta_target)
            sim.simxSynchronousTrigger(client_id)
            sim.simxGetPingTime(client_id)
            current_pos_actual = robot.forward_kinematics(theta_feedback)[:3, 3]
    ...
    if theta_feedback is None:
        theta_feedback = theta_reference.copy()

    summary = summarize_noise_run(
        label=f"{METHOD_LABEL} live {noise_channel} {noise_profile} level={noise_level:g} trial={trial}",
        history=history,
        theta_initial=theta_reference,
        theta_final=theta_feedback,
    ...
    if save_artifacts:
        ...
        save_figures(
            history,
            output_dir,
            f"{METHOD_LABEL} live {noise_channel} {noise_profile} {noise_level:g}",
            theta_reference,
            theta_feedback,
            f"live_{noise_channel}_{noise_profile}_{noise_level:g}_trial{trial}",
        )
        (output_dir / f"{OUTPUT_STEM}_live_history.json").write_text(
            json.dumps(history, indent=2, ensure_ascii=False),
            encoding="utf-8",
        )
        (output_dir / f"{OUTPUT_STEM}_live_summary.json").write_text(
...
def run_live_reference_sweep(args):
    robot = UR3eKinematics()
    cfg = make_cfg(args.duration, args.tau, args)
    result_root = Path(args.output_root) if args.output_root else build_run_output_root(RESULTS_DIR, args, "live")
    save_run_config(result_root, args, "live")
    live_root = result_root / "live_linear_reference"
    ...
    profile = "linear"
    live_levels = {
        "sensor": args.live_sensor_levels,
        "actuator": args.live_actuator_levels,
    }

    for channel in args.channels:
        for level in live_levels[channel]:
            print(f"[live] channel={channel} profile={profile} level={level:.1e}", flush=True)
            trial_summaries = []
            for trial in range(args.live_trials):
                seed = args.seed + 2_000_000 + abs(hash(("live", channel, profile, level, trial))) % 1_000_000
                output_dir = live_root / "representative" / f"{channel}_{profile}_{level:g}_trial{trial}"
                summary = run_live_noise_trial(
                    cfg=cfg,
                    robot=robot,
                    ...
                    save_artifacts=not args.skip_representative_figures,
                )
                trial_summaries.append(summary)
                all_trial_rows.append(summary)
            aggregate_rows.append(aggregate_summaries(trial_summaries))

    write_csv(live_root / "live_noise_trials.csv", all_trial_rows)
    write_csv(live_root / "live_noise_summary.csv", aggregate_rows)
    (live_root / "live_noise_summary.json").write_text(
        json.dumps(aggregate_rows, indent=2, ensure_ascii=False),
        encoding="utf-8",
    )
    plot_all_summaries(aggregate_rows, live_root / "plots")
...
def main():
    ...
    if args.output_root is None:
        args.output_root = str(build_run_output_root(RESULTS_DIR, args, "live"))
    rows = run_live_reference_sweep(args)
    result_root = Path(args.output_root)
    print(json.dumps({"status": "ok", "rows": len(rows), "result_root": str(result_root / "live_linear_reference")}, indent=2, ensure_ascii=False))
