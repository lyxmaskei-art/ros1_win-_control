...
METHOD_NAME = "Method 3c: position-and-drift objective with strict DLCCZNN solver"
OUTPUT_STEM = "method3c_position_and_drift_lccznn"
...
@dataclass
class Method3cConfig:
    duration: float = 10.0
    tau: float = 0.005
    heart_scale: float = 0.008
    task_gain: float = 198.0
...
class Method3cController:
    ...
    def build_qp_terms(self, theta_current, current_pos, desired_pos, desired_vel, jacobian_task):
        ...
        return q_matrix, q_vector, position_error, drift_delta

    def run_dlccznn_inner_solver(self, q_matrix, q_vector, lower, upper):
        z = np.clip(self.theta_dot_state.copy(), lower, upper)
        inner_tau = self.cfg.tau / float(max(int(self.cfg.solver_substeps), 1))
        z_raw = z.copy()
        residual = q_matrix @ z + q_vector
        ...
        for _ in range(max(int(self.cfg.solver_substeps), 1)):
            ...
            z_raw = z - inner_tau * correction * (direction / denom)
            z = np.clip(z_raw, lower, upper)
            if not np.all(np.isfinite(z)):
                raise FloatingPointError("Method 3c DLCCZNN solver diverged; reduce solver gain.")

        self.theta_dot_state = z.copy()
        final_residual = q_matrix @ z + q_vector
        return {
            "theta_dot": z,
            "theta_dot_raw": z_raw,
    ...
    def step(self, theta_current, current_pos, desired_pos, desired_vel, jacobian_task, lower, upper):
        ...
        q_matrix, q_vector, position_error, drift_delta = self.build_qp_terms(
            theta_current,
            current_pos,
            desired_pos,
            desired_vel,
        ...
        solver_output = self.run_dlccznn_inner_solver(q_matrix, q_vector, lower, upper)
...
def plot_inner_residual_figure(history, output_path, title_prefix):
    ...
    ax.set_title(f"{title_prefix} Inner Residual Norm")
    ax.set_xlabel("t (s)")
    ax.set_ylabel(r"$||Q_k \dot q + c_k||_2$")
    ax.grid(True, which="both", alpha=0.25)
    fig.tight_layout()
    fig.savefig(output_path, dpi=240, bbox_inches="tight")
    plt.close(fig)
...
def run_live_experiment(cfg, robot, output_dir):
    controller = Method3cController(cfg)
    ...
    summary = summarize_run("Method 3c live", history, theta_reference, theta_current, output_dir, port=port)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Integrated Method 3c DLCCZNN project runner.")
    ...
    parser.add_argument("--task-gain", type=float, default=198.0)
